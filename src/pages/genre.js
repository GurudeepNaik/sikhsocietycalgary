...
import { ProviderTable } from "src/sections/Genre/providers-table";
import { ProviderSearch } from "src/sections/Genre/provider-search";
import { useAuth } from "src/hooks/use-auth";
import { AddIndustry } from "src/sections/Genre/AddProvider";
import ListBulletIcon from "@heroicons/react/24/solid/ListBulletIcon";
import { applyPagination } from "src/utils/apply-pagination";

const Page = () => {
  const { getGenre, postGenre, deleteGenre, genre } = useAuth();
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(5);
  const [component, setComponent] = useState("INDUSTRIES");
  const useCustomers = (page, rowsPerPage) => {
    return useMemo(() => {
      return applyPagination(genre, page, rowsPerPage);
    }, [page, rowsPerPage, genre]);
  };
  ...
  return (
    <>
      <Head>
        <title>Genre | Circle Connect</title>
      </Head>
      <Box
        component="main"
        sx={{
