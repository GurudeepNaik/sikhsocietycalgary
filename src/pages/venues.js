...
import { ProviderTable } from 'src/sections/Venue/providers-table';
import { ProviderSearch } from 'src/sections/Venue/provider-search';
import { useAuth } from 'src/hooks/use-auth';
import { AddProvider } from 'src/sections/Venue/AddProvider';
import { ProviderDetails } from 'src/sections/Venue/ProviderDetails';
import ListBulletIcon from '@heroicons/react/24/solid/ListBulletIcon';

const Page = () => {
  const auth = useAuth();
  const {getVenues,venues}=auth
  ...
  useEffect(()=>{
    setData(venues?.data||[])
    // setLastPage(venues?.last_page||1)
    setTotal(venues?.total||0)
  },[venues?.current_page , venues])
